...
from .devioctl import CTL_CODE, FILE_DEVICE_CONTROLLER, METHOD_BUFFERED, FILE_READ_ACCESS, FILE_WRITE_ACCESS, \
    FILE_ANY_ACCESS
...
IOCTL_SCSI_PASS_THROUGH_DIRECT = CTL_CODE(IOCTL_SCSI_BASE, 0x0405, METHOD_BUFFERED,
...
IOCTL_MINIPORT_PROCESS_SERVICE_IRP = CTL_CODE(IOCTL_SCSI_BASE, 0x040e, METHOD_BUFFERED,
                                              FILE_READ_ACCESS | FILE_WRITE_ACCESS)
IOCTL_MPIO_PASS_THROUGH_PATH = CTL_CODE(IOCTL_SCSI_BASE, 0x040f, METHOD_BUFFERED, FILE_READ_ACCESS | FILE_WRITE_ACCESS)
IOCTL_MPIO_PASS_THROUGH_PATH_DIRECT = CTL_CODE(IOCTL_SCSI_BASE, 0x0410, METHOD_BUFFERED,
                                               FILE_READ_ACCESS | FILE_WRITE_ACCESS)

IOCTL_SCSI_PASS_THROUGH_EX = CTL_CODE(IOCTL_SCSI_BASE, 0x0411, METHOD_BUFFERED, FILE_READ_ACCESS | FILE_WRITE_ACCESS)
IOCTL_SCSI_PASS_THROUGH_DIRECT_EX = CTL_CODE(IOCTL_SCSI_BASE, 0x0412, METHOD_BUFFERED,
                                             FILE_READ_ACCESS | FILE_WRITE_ACCESS)

IOCTL_MPIO_PASS_THROUGH_PATH_EX = CTL_CODE(IOCTL_SCSI_BASE, 0x0413, METHOD_BUFFERED,
                                           FILE_READ_ACCESS | FILE_WRITE_ACCESS)
IOCTL_MPIO_PASS_THROUGH_PATH_DIRECT_EX = CTL_CODE(IOCTL_SCSI_BASE, 0x0414, METHOD_BUFFERED,
                                                  FILE_READ_ACCESS | FILE_WRITE_ACCESS)
...
class SCSI_PASS_THROUGH_WITH_BUFFERS(Structure):
    ...
    _fields_ = [
        ...
        ("Cdb", UCHAR * 16),
        ("Filler", ULONG),
        ("ucSenseBuf", UCHAR * SPT_SENSE_LENGTH),
        ("ucDataBuf", UCHAR * SPTWB_DATA_LENGTH),
    ]
